...
posAndShape :: MonadMoment m => Event Float -> [Asteroid] -> m (Behavior [Asteroid])
posAndShape eTick startAsteroid =
  accumB startAsteroid $ (\dt -> Optics.traversed % Asteroid.physics %~ Physics.update dt []) <$> eTick
